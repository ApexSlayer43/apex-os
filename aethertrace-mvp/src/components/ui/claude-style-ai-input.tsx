...
const DEFAULT_MODELS_INTERNAL: ModelOption[] = [
  { id: "aethertrace-intelligence", name: "AetherTrace Intelligence", description: "Evidence analysis & structuring", badge: "Default" },
];
...
const isTextualFile = (file: File): boolean => {
  ...
  const textualExtensions = ["txt","md","py","js","ts","jsx","tsx","html","htm","css","scss","json","xml","yaml","yml","csv","sql","sh","log"];
...
const FilePreviewCard: React.FC<{ file: FileWithPreview; onRemove: (id: string) => void }> = ({ file, onRemove }) => {
  ...
  return (
    <div className={cn("relative group bg-zinc-700 border w-fit border-zinc-600 rounded-lg p-3 size-[125px] shadow-md flex-shrink-0 overflow-hidden", isImage ? "p-0" : "p-3")}>
...
const PastedContentCard: React.FC<{ content: PastedContent; onRemove: (id: string) => void }> = ({ content, onRemove }) => {
  ...
  return (
    <div className="bg-zinc-700 border border-zinc-600 relative rounded-lg p-3 size-[125px] shadow-md flex-shrink-0 overflow-hidden">
      <div className="text-[8px] text-zinc-300 whitespace-pre-wrap break-words max-h-24 overflow-y-auto">{needsTruncation ? previewText + "..." : content.content}</div>
      <div className="group absolute flex justify-start items-end p-2 inset-0 bg-gradient-to-b to-[#30302E] from-transparent overflow-hidden">
        ...
        <div className="group-hover:opacity-100 opacity-0 transition-opacity duration-300 flex items-center gap-0.5 absolute top-2 right-2">
          <Button size="icon" variant="outline" className="size-6" onClick={() => navigator.clipboard.writeText(content.content)} title="Copy"><Copy className="h-3 w-3" /></Button>
          <Button size="icon" variant="outline" className="size-6" onClick={() => onRemove(content.id)} title="Remove"><X className="h-3 w-3" /></Button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-zinc-700 border border-zinc-600 relative rounded-lg p-3 size-[125px] shadow-md flex-shrink-0 overflow-hidden">
      <div className="text-[8px] text-zinc-300 whitespace-pre-wrap break-words max-h-24 overflow-y-auto">
        {file.textContent ? (needsTruncation ? previewText + "..." : file.textContent) : <div className="flex items-center justify-center h-full text-zinc-400"><Loader2 className="h-4 w-4 animate-spin" /></div>}
      </div>
      <div className="group absolute flex justify-start items-end p-2 inset-0 bg-gradient-to-b to-[#30302E] from-transparent overflow-hidden">
        <p className="capitalize text-white text-xs bg-zinc-800 border border-zinc-700 px-2 py-1 rounded-md">{fileExtension}</p>
        <div className="group-hover:opacity-100 opacity-0 transition-opacity duration-300 flex items-center gap-0.5 absolute top-2 right-2">
          {file.textContent && <Button size="icon" variant="outline" className="size-6" onClick={() => navigator.clipboard.writeText(file.textContent || "")} title="Copy"><Copy className="h-3 w-3" /></Button>}
          <Button size="icon" variant="outline" className="size-6" onClick={() => onRemove(file.id)} title="Remove"><X className="h-3 w-3" /></Button>
        </div>
      </div>
    </div>
  );
...
export const ClaudeChatInput: React.FC<ChatInputProps> = ({
  ...
  const handleFileSelect = useCallback((selectedFiles: FileList | null) => {
    ...
    const filesToAdd = Array.from(selectedFiles).slice(0, availableSlots).filter((file) => file.size <= maxFileSize);
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    if (textData && textData.length > PASTE_THRESHOLD && pastedContent.length < 5) {
      ...
      setPastedContent((prev) => [...prev, { id: String(Math.random()), content: textData, timestamp: new Date(), wordCount: textData.split(/\s+/).filter(Boolean).length }]);
    }
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto" onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={(e) => { e.preventDefault(); setIsDragging(false); }} onDrop={(e) => { e.preventDefault(); setIsDragging(false); if (e.dataTransfer.files) handleFileSelect(e.dataTransfer.files); }}>
      ...
      <div style={{ background: 'rgba(200,212,228,0.04)', border: '1px solid rgba(200,212,228,0.08)', borderRadius: 16 }} className="shadow-lg items-end gap-2 min-h-[150px] flex flex-col">
        <textarea
          ref={textareaRef}
          value={message}
          ...
          disabled={disabled}
          className="flex-1 min-h-[100px] w-full p-4 focus:outline-none focus:ring-0 focus:border-none border-none outline-none ring-0 max-h-[120px] resize-none bg-transparent text-zinc-100 focus-visible:ring-0 focus-visible:outline-none placeholder:text-zinc-500 text-sm sm:text-base"
          style={{ caretColor: '#C8D4E0', outline: 'none', boxShadow: 'none' }}
          rows={1}
        />
        <div className="flex items-center gap-2 justify-between w-full px-3 pb-2">
          <div className="flex items-center gap-2">
            <button
              className="h-9 w-9 p-0 flex items-center justify-center rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-white/5 transition-colors"
          ...
          <div className="flex items-center gap-2">
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'rgba(200,212,228,0.3)', letterSpacing: '0.04em' }}>
              AetherTrace Intelligence
            </span>
            <button
              className={cn(
                "h-9 w-9 p-0 flex-shrink-0 rounded-lg flex items-center justify-center transition-colors",
                canSend ? "bg-white text-black hover:bg-white/90" : "bg-white/5 text-zinc-500 cursor-not-allowed"
              )}
              onClick={handleSend}
              disabled={!canSend}
              title="Seal evidence"
        ...
        {(files.length > 0 || pastedContent.length > 0) && (
          <div className="overflow-x-auto p-3 w-full" style={{ borderTop: '1px solid rgba(200,212,228,0.06)', background: 'rgba(200,212,228,0.02)' }}>
